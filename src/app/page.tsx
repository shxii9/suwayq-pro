...
export default function Home() {
  const listings = [
    { id: "1", title: "آيفون 15 برو ماكس", price: 320, location: "الكويت العاصمة", image: "https://images.unsplash.com/photo-1696446701796-da61225697cc?w=500" },
      ...
      <main className="container mx-auto px-4 pt-32 pb-20">
        ...
        <div className="max-w-4xl mx-auto mb-16 relative group">
          ...
          <div className="relative bg-white/80 dark:bg-gray-900/80 backdrop-blur-2xl p-3 rounded-[2.2rem] border border-white/20 dark:border-gray-800 flex flex-col md:flex-row gap-3 shadow-2xl">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="ابحث عن سيارة، هاتف، أو عقار..." className="w-full h-16 pr-14 bg-transparent font-bold outline-none dark:text-white text-lg placeholder:text-slate-400" />
            </div>
            <div className="flex gap-2">
              <button className="h-16 w-16 flex items-center justify-center bg-slate-100 dark:bg-gray-800 rounded-3xl text-slate-500 hover:text-blue-600 transition-all hover:rotate-12"><Mic size={24} /></button>
              <button className="h-16 px-10 bg-blue-600 hover:bg-blue-700 text-white rounded-3xl font-black text-lg transition-all shadow-xl shadow-blue-500/20 active:scale-95">ابحث الآن</button>
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-4 overflow-x-auto pb-12 no-scrollbar justify-start md:justify-center">
          {categories.map((cat, i) => (
            <button key={i} className={`flex items-center gap-3 px-8 py-4 rounded-[1.5rem] whitespace-nowrap font-black transition-all hover:scale-105 border border-transparent hover:border-blue-500/30 shadow-sm ${cat.color}`}>
        ...
        <div className="flex items-center justify-between mb-8">
          <Link href="/search" className="text-blue-600 dark:text-blue-400 font-black text-sm flex items-center gap-1 hover:underline">عرض الكل ←</Link>
          <div className="flex items-center gap-2 font-black text-xl text-slate-800 dark:text-white">
             أحدث الإعلانات <TrendingUp className="text-orange-500" />
          </div>
        </div>
