          ...
          <tbody className="divide-y divide-slate-700 text-slate-300">
            {users.map((user) => (
              <tr key={user.id} className="hover:bg-slate-700/50 transition">
                ...
                <td className="p-4">
                  <span
                    className={`px-2 py-1 rounded text-xs font-bold ${user.status === "Active" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
                  >
                    {user.status}
                  </span>
                </td>
                <td className="p-4 flex gap-2">
                  ...
                  {user.status === "Active" && (
                    <button className="text-orange-500 hover:text-orange-300 p-1 rounded">
                      <ShieldOff size={16} />
                    </button>
                  )}
                </td>
