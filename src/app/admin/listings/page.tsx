          ...
          <tbody className="divide-y divide-slate-700 text-slate-300">
            {listings.map((item) => (
              <tr key={item.id} className="hover:bg-slate-700/50 transition">
                ...
                <td className="p-4">
                  <span
                    className={`px-2 py-1 rounded text-xs font-bold ${item.status === "Active" ? "bg-green-500/20 text-green-400" : item.status === "Pending" ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"}`}
                  >
                    {item.status}
                  </span>
                </td>
                <td className="p-4 flex gap-2">
                  {item.status === "Pending" && (
                    <button className="text-green-500 hover:text-green-300 p-1 rounded">
                      <CheckCircle size={16} />
                    </button>
                  )}
                  <button className="text-red-500 hover:text-red-300 p-1 rounded">
                    <Trash2 size={16} />
                  </button>
                </td>
              </tr>
